...
def verify(request,email,password,token,container,ready,passed,sql,schema):
    ...
    try:
        ...
        result=subprocess.run(['node',str(root/'scripts/smoke_financial_history_browser_homolog.cjs')],
        ...
        if result.returncode:
            ...
            stages=[line for line in result.stderr.splitlines() if line.startswith('Financial history browser failed at stage: ')]
