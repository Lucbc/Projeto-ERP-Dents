...
def verify(request,email,password,token,container,ready,passed,sql,schema, *, browser_script='smoke_financial_history_browser_homolog.cjs'):
    ...
    try:
        ...
        if result.returncode:
            ...
            stages=[line for line in result.stderr.splitlines() if line.startswith(('Financial history browser failed at stage: ', 'Catalog deletion browser failed at stage: ', 'Appointment deletion browser failed at stage: ', 'Dentist deletion browser failed at stage: '))]
