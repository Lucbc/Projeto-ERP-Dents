...
- Segundo CI falhou em três testes de disputa de agenda: `40P01` por deadlock nas restrições GiST, em vez de `23P01`. A API já retorna 409 para ambos; corrigida expectativa dos testes, reforçando conferência de registros/versões e rollback do perdedor. Smoke HTTP aceita as duas mensagens seguras e mantém zero sobreposições. Ajustes somente de testes; 11 focados e 12 grupos HTTP aprovados localmente. Publicar e acompanhar novo CI antes de fechar.
