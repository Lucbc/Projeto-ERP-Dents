...
**Atualização 2B.7.2 concluída em 26/09/2026:** consultas relêem disponibilidade sob bloqueio compartilhado do dentista, com versão e gravação na mesma transação. Redução/inativação incompatível é recusada até reagendar/cancelar compromissos scheduled/confirmed ainda não encerrados, conforme decisão do usuário. UI distingue disponibilidade de versão, mantém rascunho e exige recarga/revisão explícita. [Evidências](./homologacao-etapa-2B7-2.md): 262 backend/106 frontend, HTTP/Chrome e preservação aprovados; seis disputas nas duas ordens, manutenção histórica e rollback verificados. R18 permanece parcial para administração de usuários/permissões, próximo diagnóstico 2B.8. Registros abaixo são históricos.

**Atualização 2B.7.1 concluída:** horários reais validados no schema/domínio/formulário; agenda compara segundos/microssegundos e leitura legada permite correção explícita sem truncamento. [Evidências](./homologacao-etapa-2B7-1.md): 254 backend/102 frontend no CI, HTTP/Chrome e preservação aprovados. A corrida disponibilidade × consulta ainda exige coordenação transacional na 2B.7.2; regra sobre compromissos existentes aguarda resposta. R18 permanece parcial.

**Diagnóstico 2B.7, 25/09/2026:** [seis interleavings em duas sessões](./plano-etapa-2B7.md) confirmaram consulta criada/reagendada/reativada após redução de horário ou inativação já confirmada do dentista. Disponibilidade é validada fora da escrita transacional. Também identificados segundos descartados e horários 25:00–26:00 aceitos no schema. Sem correção funcional nesta preparação; política de compromissos existentes aguarda escolha do usuário. Próximo recorte independente: 2B.7.1, validação de horários. R18 permanece parcial.

**Atualização 2B.6.4.2 concluída:** exclusão individual usa resultado transacional para 404 e rollback completo; interface confirma paciente/arquivo, exige recarga após erro e preserva upload/prévia conforme o resultado. Download verifica e envia pelo mesmo descritor, mantém Range e fecha em erro/desconexão; limpeza pode prosseguir sem interromper bytes já abertos ou reter lock global. [Evidências](./homologacao-etapa-2B6-4-2.md): 247 backend/97 frontend no CI, HTTP/Chrome e preservação aprovados. Exclusões previstas na 2B.6 concluídas; R18 continua parcial para disponibilidade × agendamento e outras regras entre recursos.

**Preparação 2B.6.4.2, 25/09/2026:** [diagnóstico controlado](./plano-etapa-2B6-4-2.md) reproduziu download × limpeza: 500 antes da resposta ou interrupção após cabeçalhos 200. Mesmo descritor verificado pelo antivírus manteve bytes íntegros após remover o caminho. Caso de uso ignora exclusão transacional que retorna falso, confirmado com repositório substituído. Sete diagnósticos, sem teste novo de disputa no banco/HTTP autenticado/Chrome; correção pendente e R18 continua parcial.

**Atualização 2B.6.4.1 concluída:** exclusão do paciente exige versão e confirmação do conjunto de exames, comparadas sob lock antes da cascata. Troca de exame com mesma quantidade invalida a confirmação; paciente com exames exige também `exams.delete`. Interface confirma efeitos, preserva rascunho e exige recarga/nova confirmação em falha. [Evidências](./homologacao-etapa-2B6-4-1.md): 235 backend/88 frontend, HTTP/Chrome/CI aprovados; fila, histórico financeiro e dados principais preservados. R18 permanece parcial para a exclusão individual de exames e regras entre recursos; download × limpeza fica na 2B.6.4.2.

**Preparação 2B.6.4:** diagnóstico isolado confirmou que DELETE de paciente ignora versão e pode apagar um exame novo mesmo com quantidade inalterada. Também confirmou cascata sem `exams.delete`; consultas continuam impeditivas e rollback conserva arquivos/fila. [Contrato](./plano-etapa-2B6-4.md): prévia consistente com fingerprint do conjunto, versão e permissão explícita da cascata. Correção ainda pendente; download × limpeza é risco por inspeção a reproduzir na 2B.6.4.2. Não contar a preparação como resolução de R18.

**Atualização 2B.6.3.1 concluída:** exclusão de dentistas exige versão e comparação transacional; contas vinculadas, inclusive inativas, impedem a exclusão por FK RESTRICT. Consultas e histórico financeiro preservados, reatribuição de conta permanece explícita pela administração. Interface exige recarga e nova confirmação após erro. [Evidências](./homologacao-etapa-2B6-3-1.md): CI 222 backend/81 frontend, HTTP, Chrome e preservação aprovados; auditoria das seis imagens aprovada após correção de libexpat. R18 permanece parcial para pacientes/exames e regras entre recursos.

**Atualização 2B.6.2.1 concluída:** exclusão de consultas na lista/calendário exige versão e comparação transacional antes de remover procedimentos vinculados. Confirmação identifica consulta salva e efeitos financeiros; erro preserva rascunho e exige revisão explícita/nova confirmação. Disputas com geração/baixa e preservação de pagamentos, referências e recibos aprovadas. [Evidências](./homologacao-etapa-2B6-2-1.md): CI 209 backend/76 frontend, HTTP e Chrome, dados anteriores preservados. R18 permanece parcial para dentistas, pacientes/exames e regras entre recursos.
...
**Atualização parcial 2B.1/2B.2/2B.3/2B.4:** edição de pacientes, consultas, dentistas, procedimentos e especialidades usa versão obrigatória e atualização atômica; rascunho preservado e recarga explícita no frontend. Evidências de [pacientes](./homologacao-etapa-2B1.md), [agenda](./homologacao-etapa-2B2.md), [dentistas](./homologacao-etapa-2B3.md), [procedimentos](./homologacao-etapa-2B4-1.md) e [especialidades](./homologacao-etapa-2B4-2.md), com estado de validação em cada relatório. Demais recursos, exclusões e regras entre disponibilidade e consultas continuam pendentes; descrição original abaixo mantida como histórico.
